...
impl Tutor {
    pub fn new() -> Self {
        ...
        let mg_pesto = [&MG_PAWN, &MG_KNIGHT, &MG_BISHOP, &MG_ROOK, &MG_QUEEN, &MG_KING];
        let eg_pesto = [&EG_PAWN, &EG_KNIGHT, &EG_BISHOP, &EG_ROOK, &EG_QUEEN, &EG_KING];

        for p_idx in 0..6 {
            let mg_val = MG_VALUE[p_idx];
            let eg_val = EG_VALUE[p_idx];

            for sq in 0..64 {
                let white_pc = p_idx * 2;
                let black_pc = p_idx * 2 + 1;

                // White
                mg_tables[white_pc][sq] = mg_val + mg_pesto[p_idx][sq];
                eg_tables[white_pc][sq] = eg_val + eg_pesto[p_idx][sq];

                // Black (flip square vertically)
                let flip_sq = sq ^ 56;
                mg_tables[black_pc][sq] = mg_val + mg_pesto[p_idx][flip_sq];
                eg_tables[black_pc][sq] = eg_val + eg_pesto[p_idx][flip_sq];
            }
        }

        Self { mg_tables, eg_tables }
    }
    ...
    pub fn evaluate(&self, board: &Board) -> i32 {
        ...
        let side_to_move = board.side_to_move() as usize;
        let other_side = side_to_move ^ 1;

        let mg_score = mg[side_to_move] - mg[other_side];
        let eg_score = eg[side_to_move] - eg[other_side];

        let mut mg_phase = game_phase;
        if mg_phase > MAX_PHASE {
            mg_phase = MAX_PHASE;
        }
...
mod tests {
    ...
    #[test]
    fn test_initial_board_eval() {
        let tutor = Tutor::new();
        let board = Board::default();
        let eval = tutor.evaluate(&board);
