...
impl<'a> SearchHandler<'a> {
    pub fn new(engine: &'a Engine, tutor: &'a Tutor, tt: &'a TranspositionTable, board: Board, stop_flag: Arc<AtomicBool>, thread_id: usize) -> Self {
        Self {
            engine,
            tutor,
            tt,
    ...
    pub fn search(&mut self, max_depth: u8) -> (Option<ChessMove>, i32) {
        ...
        let (root_engine_state, root_policy, root_value) = self.engine.get_initial_state(&tokens).unwrap();
        ...
        for depth in 1..=max_depth {
            ...
            let score = self.negamax(depth, -INFINITY, INFINITY, root_engine_state.clone(), root_value, Some(&root_policy));

            if self.should_stop() && depth > 1 { break; }
            ...
            if self.thread_id == 0 {
                ...
                println!("info depth {} score cp {} nodes {} nps {} time {} pv {}",
                    depth, best_score, self.nodes_searched, nps, elapsed, best_move.map(|m| m.to_string()).unwrap_or_default());
            }
        }
    ...
    fn negamax(&mut self, depth: u8, mut alpha: i32, mut beta: i32, engine_state: EngineState, ai_value: f32, policy: Option<&Tensor>) -> i32 {
        // Periodic time check
        ...
        if status == chess::BoardStatus::Checkmate { return -MATE_SCORE + self.ply as i32; }
        ...
        if !in_check && self.ply > 0 {
            ...
            if depth >= 3 && static_score >= beta {
                if let Some(null_board) = self.game_state.board.null_move() {
                    ...
                    self.ply += 1;
                    let r = 3 + depth / 6;
                    let score = -self.negamax(depth.saturating_sub(r), -beta, -beta + 1, engine_state.clone(), ai_value, None);
                    self.ply -= 1;
        ...
        let mut scored_moves: Vec<(ChessMove, i32)> = moves.map(|mv| {
            let mut score = 0;
            if Some(mv) == tt_move { score = 2_000_000; }
                ...
                score = 1_000_000 + crate::tutor::MG_VALUE[victim.to_index()] * 10 - crate::tutor::MG_VALUE[attacker.to_index()];
            } else {
                if self.ply < 64 {
                    if self.killers[self.ply][0] == Some(mv) { score = 900_000; }
                    else if self.killers[self.ply][1] == Some(mv) { score = 800_000; }
                }
        ...
        for (i, (mv, _)) in scored_moves.into_iter().enumerate() {
            ...
            if depth >= 2 {
                ...
                if let Ok((p, v)) = self.engine.forward_incremental(token, &mut next_engine_state) {
                    next_ai_value = v;
                    next_policy = Some(p);
                }
            }
            ...
            let score = if i == 0 {
                -self.negamax(depth - 1, -beta, -alpha, next_engine_state, next_ai_value, next_policy.as_ref())
            } else {
                let reduction = if depth >= 3 && i >= 3 && !in_check && board_backup.piece_on(mv.get_dest()).is_none() {
                    1 + (i / 8) as u8
                } else { 0 };

                let mut s = -self.negamax(depth.saturating_sub(1 + reduction), -alpha - 1, -alpha, next_engine_state.clone(), next_ai_value, next_policy.as_ref());
                if s > alpha && reduction > 0 {
                    s = -self.negamax(depth - 1, -beta, -alpha, next_engine_state, next_ai_value, next_policy.as_ref());
                }
                s
            };
...
impl Default for TimeControl {
    fn default() -> Self {
        Self {
            ...
            movestogo: None, movetime: None, infinite: false, depth: None,
        }
    }
}
...
impl TimeControl {
    pub fn calculate_time(&self, is_white: bool) -> Duration {
        if self.infinite { return Duration::from_secs(3600); }
        if let Some(mt) = self.movetime { return Duration::from_millis(mt.saturating_sub(OVERHEAD_MS)); }
