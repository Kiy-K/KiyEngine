    ...
    fn alpha_beta(&mut self, board: &Board, depth: u8, mut alpha: i32, mut beta: i32) -> i32 {
        ...
        if depth >= 3 && !in_check && static_score >= beta && !is_root {
            ...
            if let Some(null_board) = board.null_move() {
                self.ply += 1;
                let null_score =
                    -self.alpha_beta(&null_board, depth.saturating_sub(r), -beta, -beta + 1);
                self.ply -= 1;
        ...
        let mut scored_moves: Vec<(ChessMove, i32)> = moves
            .map(|mv| {
                ...
                if Some(mv) == tt_move {
                    ...
                    score = 100_000 + crate::tutor::MG_VALUE[victim.to_index()] * 10
...
impl<'a> SearchHandler<'a> {
        ...
        for depth in 1..=max_depth {
            ...
            if let Some(last) = self.last_eval {
                if last - score > PANIC_THRESHOLD {
                    ...
                    println!(
                        "info string PANIC MODE: eval dropped {}cp, extending time",
                        last - score
                    );
                }
            }
    ...
    fn neural_hard_truth_search(&mut self) -> (Option<ChessMove>, i32) {
        ...
        for result in &results {
            let status = if result.is_vetoed {
                "VETOED"
            } else {
                "Accepted"
            };
    ...
    fn extract_top_moves(
        &self,
        board: &Board,
        policy_logits: &candle_core::Tensor,
        top_n: usize,
    ) -> Vec<(ChessMove, f32)> {
    ...
    fn score_move(&self, mv: &ChessMove, tt_move: Option<ChessMove>) -> i32 {
        ...
        if let Some(victim) = board.piece_on(mv.get_dest()) {
            ...
            return 100_000 + crate::tutor::MG_VALUE[victim.to_index()] * 10
                - crate::tutor::MG_VALUE[attacker.to_index()];
        }
