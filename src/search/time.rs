...
impl TimeManager {
    pub fn new(wtime: u32, btime: u32, winc: u32, binc: u32, movestogo: u32, move_overhead_ms: u32, is_white: bool) -> Self {
        let (time, inc) = if is_white { (wtime, winc) } else { (btime, binc) };
