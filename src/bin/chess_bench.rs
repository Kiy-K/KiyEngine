...
fn main() -> anyhow::Result<()> {
    ...
    let test_positions = vec![
        ...
        ("TACTICAL_FORK", "r3k2r/ppp2ppp/2n5/3p4/2B1P3/2N5/PPP2PPP/R3K2R w KQkq - 0 1"),
    ...
    for (name, fen) in test_positions {
        ...
        match rx.recv_timeout(Duration::from_secs(30)) {
            Ok(msg) => {
                ...
                let mv = ChessMove::from_str(move_str).map_err(|e| anyhow::anyhow!("Move error: {:?}", e))?;
                if board.legal(mv) {
                    println!("Status: ✅ LEGAL MOVE");
                } else {
                    println!("Status: ❌ ILLEGAL MOVE PRODUCED!");
                    return Err(anyhow::anyhow!("Engine produced illegal move {} in pos {}", move_str, name));
                }
